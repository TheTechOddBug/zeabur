...
A newly created Team is **inactive** — until an administrator activates the Team plan, no one in the Team can create projects, deploy services, manage Team servers, or send emails through Z-Send.
...
Click **Continue to payment**. The Team's subscription becomes active right away — projects, deploys, servers, and Z-Send unlock immediately.
</Steps>
...
Team credits are scoped to the Team — they offset only **Team-owned** invoices and server bills. They cannot be transferred to a member's personal account, and a member's personal credits cannot be applied to a Team invoice.
